    ...
    with open(inputVCF) as file:
        ...
        for line in file:
            ...
            if line.startswith('#'):
                ...
                if sv_type == "DEL" or sv_type == "INV":
                    end_on_chr = start_on_chr + length
                    end_on_graph = end_on_chr
                
                elif sv_type == "INS":
                    end_on_chr = start_on_chr
                    end_on_graph = start_on_chr + length
                ...
                if sv_id in dict_of_close_sv[chrom]:
                    ...
                    region_id = '_'.join(['ref', chrom, '-'.join([sv_type, pos, str(end_on_graph)])]) #= 'ref_' + chrom + '_' + sv_type + "-" + pos_sv_1 + '-' + end_sv_1 (ex: 'ref_1_DEL-88749-90149')
                    ...
                    converted_vcf_lines = convert_sv(line, 'region_id', converted_start, converted_end, length, l_adj, cut)

                #Following close SVs
                elif found_close_sv and (sv_id in dict_of_close_sv[chrom][first_close_sv]):
                    #Get left flanking region + sv ref seq
                    region_id = '_'.join([region_id, '-'.join([sv_type, pos, str(end_on_graph)])]) #ex: 'ref_1_DEL-88749-1400' + '_' + sv2_type + '-' + pos_sv_2 + '-' + end_sv_2
                    ...
                    if sv_type == "DEL" or sv_type == "INV":
                        ...
                        converted_end = converted_start
                    converted_vcf_lines = converted_vcf_lines + convert_sv(line, 'region_id', converted_start, converted_end, length, l_adj, cut)
                ...
                #SV not in sequence of close SV
                    ...
                    region_id = '_'.join(['ref', chrom, '-'.join([sv_type, pos, str(end_on_graph)])])
                    converted_start = l_adj
...
def convert_sv(sv_line, region_id, converted_start, converted_end, length, l_adj, cut):
