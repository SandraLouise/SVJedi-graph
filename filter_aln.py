...
def fill_sv_nodes(node_dict, sv_region, nodes, dict_of_nodes_len):
    ...
    for sv in range(1, sv_number + 1):
        ...
        for node in nodes:
            ...
            #         #Get allele specific nodes
            #         if (pos_on_graph > sv_start - 1) and (pos_on_graph < sv_end + 1): #node contains sv

            #             if sv_type == 'DEL':
            #                 node_dict[sv_id][1].append(node) #add node to reference allele specific nodes

            #             elif sv_type == 'INS':
            #                 node_dict[sv_id][2].append(node) #add node to alternative allele specific nodes
